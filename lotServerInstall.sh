...
function Install()
{
  ...
  URLKernel='http://github.itzmx.com/1265578519/lotServer/master/lotServer.log'
  ...
  mkdir -p "${AcceTmp}/bin/"
  mkdir -p "${AcceTmp}/etc/"
  wget --no-check-certificate -qO "${AcceTmp}/bin/${AcceBin}" "https://raw.githubusercontent.com/1265578519/lotServer/master/${MyKernel}"
  [ ! -f "${AcceTmp}/bin/${AcceBin}" ] && Uninstall "Download Error! Not Found ${AcceBin}. "
  Welcome;
  wget --no-check-certificate -qO "/tmp/lotServer.tar" "https://raw.githubusercontent.com/1265578519/lotServer/master/lotServer.tar"
  tar -xvf "/tmp/lotServer.tar" -C /tmp
  acce_ver=$(acce_check ${KNV})
  # 如果有自己搭建的或者api失效，这里修改成你自己的api
  wget --no-check-certificate -qO "${AcceTmp}/etc/apx.lic" "http://bbs.itzmx.com/1/keygen.php?ver=${acce_ver}&mac=${Mac}"
